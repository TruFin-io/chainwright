...
export async function workerScopeContextSolana({ workerInfo, profileName, slowMo }: WorkerScopeContext) {
    ...
    let page = context.pages().find((page) => page.url().startsWith(formatedIndexUrl));
